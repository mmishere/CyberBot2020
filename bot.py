...
# Retrieve BTM given BODY stat
def get_BTM(BODY: int) -> int:
...
@bot.command(name="idea", help="Get a Cyberpunk 2020 character idea.")
async def idea(ctx):
...
@bot.command(name="del2020", help="Deletes a Cyberpunk 2020 character from the database, if they're in there.")
async def del2020(ctx, handle: str):
    id = bot.user.id
    to_send = "Something went wrong" # this should never be sent
    try:
        delete_command = cursor.execute("DELETE FROM characters_2020 WHERE client=? AND handle=?", (id,handle)).fetchall()
        to_send = handle + " was deleted from Cyberpunk 2020 database!"
    except:
        to_send = handle + " not found, not deleted from Cyberpunk 2020 database!"
    await ctx.send(to_send)


@bot.command(name="delred", help="Deletes a Cyberpunk Red character from the database, if they're in there.")
async def delred(ctx, handle: str):
    id = bot.user.id
    to_send = "Something went wrong" # this should never be sent
    try:
        delete_command = cursor.execute("DELETE FROM characters_2020 WHERE client=? AND handle=?", (id,handle)).fetchall()
